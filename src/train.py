...
direc = './FC3model_first_minimize_Etot_weight_decay'
...
if resume:    # 中断的时候恢复训练
    path=r"./FC3model_first_minimize_Etot/3layers_MLFFNet_33epoch.pt"
...
for epoch in range(start_epoch, n_epoch + 1):
    ...
    print('epoch = {}, step = {}, training force RMSE loss = {:.8f}, training etot RMSE loss = {:.8f}, lr = {}, time cost = {}'.format(epoch, n_iter, train_force_rmse_loss, train_etot_rmse_loss, lr, time_cost)) 

    valid_epoch_force_RMSE_loss = 0
    valid_epoch_etot_RMSE_loss = 0
    loss_function_err = 0
    for i_batch, sample_batches in enumerate(loader_valid):
        square_error, Force_RMSE_error, Force_ABS_error, Etot_RMSE_error, Etot_ABS_error = valid(sample_batches, model, nn.MSELoss())
    ...
    print('valid force RMSE loss = {:.8f}, valid etot RMSE loss = {:.8f}, valid_total_loss = {:.8f}'.format(valid_force_rmse_loss, valid_etot_rmse_loss, valid_total_loss))


    iprint = 1 #隔几个epoch记录一次误差
    f_err_log=pm.dir_work+'out_train_weight_decay_err_relu.dat'
    if epoch // iprint == 1:
