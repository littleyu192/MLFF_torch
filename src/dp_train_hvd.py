...
model = DP(config, device, stat)
model.to(device)

if opt_horovod:
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, num_replicas=hvd.size(), rank=hvd.rank())
    loader_train = Data.DataLoader(train_dataset, batch_size=batch_size, shuffle=False, sampler=train_sampler)

    vaild_sampler = torch.utils.data.distributed.DistributedSampler(valid_dataset, num_replicas=hvd.size(), rank=hvd.rank())
    loader_valid = Data.DataLoader(valid_dataset, batch_size=batch_size, shuffle=False)
else:
    loader_train = Data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    loader_valid = Data.DataLoader(valid_dataset, batch_size=batch_size, shuffle=True)
...
for epoch in range(start_epoch, n_epoch + 1):
    ...
    loss_F = hvd.allreduce(torch.tensor(loss_F))
    loss_Etot = hvd.allreduce(torch.tensor(loss_Etot))  
