...
void PairQCAD::allocate()
{
  ...
  memory->create(setflag, n, n, "pair_qcad:setflag");
  for (int i = 1; i < n; i++)
    for (int j = i; j < n; j++) setflag[i][j] = 0;
  memory->create(cutsq,n,n,"pair_qcad:cutsq");
  memory->create(scale,n,n,"pair_qcad:scale");

  memory->create(e_atom, atom->natoms, "pair_qcad:e_atom");
  memory->create(x_cart, atom->natoms, 3, "pair_qcad:x_cart");
  memory->create(x_cart_tmp, atom->natoms, 3, "pair_qcad:x_cart_tmp");
  memory->create(x_frac, atom->natoms, 3, "pair_qcad:x_frac");
  memory->create(f_atom, atom->natoms, 3, "pair_qcad:f_atom");
  memory->create(itype_atom, atom->natoms, "pair_qcad:itype_atom");
  memory->create(itype_tmp, atom->natoms, "pair_qcad:itype_tmp");
}
...
void PairQCAD::compute(int eflag, int vflag)
{
  ...
  if (nprocs > 1000)    error->all(FLERR, "too many processes, modify pair_qcad.cpp for more processes");
