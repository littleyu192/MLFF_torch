...
def NDEKF_Etot(sample_batches,model,nlayers_embeding,nlayers_fitting,nlayers_resnet_dt,nnodes,kalman_lambda,kalman_nue,P):
    '''
    finish
    Attention！！！A should not to be layer * nodes scale!
    '''
    start=time.time()
    nlayers=nlayers_embeding+nlayers_fitting+nlayers_resnet_dt
    node_num=0
    for i in range(nlayers):
        node_num+=nnodes[i]
    H=[[] for i in range(nlayers)]
    weights=[[] for i in range(nlayers)]
    #A=[[[] for ii in range(pm.nNodes[i][0])] for i in range(pm.nLayers)]
    K=[[[] for ii in range(nnodes[i])] for i in range(nlayers)]
    weights_index=[]
    ...
    error=Etot_label.item()-Etot_predict.item()#well this is the bug.we should use item()!!!!
    error=error/pm.natoms[0]
    model_time=time.time()
    # print('model_time: ',model_time-start,'s')
    if error < 0:
        error = -1.0 * error
    ...
    tmp=0
    for i in range(nlayers_embeding):
        for name, param in model.named_parameters():
            if 'embeding_net.weights.weight{}'.format(i) in name:
                H[tmp]=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                weights[tmp]=list(param.data.chunk(param.data.shape[1],dim=1))
                weights_index.append(param.data.shape[0])
            if 'embeding_net.bias.bias{}'.format(i) in name:
                H_tmp=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                weights_tmp = list(param.data.chunk(param.data.shape[1], dim=1))
                for ii in range(len(H[tmp])):
                    H[tmp][ii]=torch.cat((H[tmp][ii], H_tmp[ii].reshape(-1,1)))
                    weights[tmp][ii]=torch.cat((weights[tmp][ii],weights_tmp[ii].reshape(-1,1)))
        tmp+=1
    for i in range(nlayers_fitting):
        for name, param in model.named_parameters():
            if 'fitting_net.weights.weight{}'.format(i) in name:
                H[tmp]=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                weights[tmp]=list(param.data.chunk(param.data.shape[1],dim=1))
                weights_index.append(param.data.shape[0])
            if 'fitting_net.bias.bias{}'.format(i) in name:
                H_tmp=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                weights_tmp = list(param.data.chunk(param.data.shape[1], dim=1))
                for ii in range(len(H[tmp])):
                    H[tmp][ii]=torch.cat((H[tmp][ii], H_tmp[ii].reshape(-1,1)))
    ...
    for i in range(nlayers_embeding):
        for name, param in model.named_parameters():
            if 'fitting_net.resnet_dt.resnet_dt{}'.format(i+1) in name:
                H[tmp]=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                weights[tmp]=list(param.data.chunk(param.data.shape[1],dim=1))
                weights_index.append(param.data.shape[0])
        tmp+=1
    get_grad_weight_time=time.time()
    # print('get_grad_weights_time: ',get_grad_weight_time-model_time,'s')
    '''
    1. get the Kalman Gain Matrix
    '''
    tmp=0
    for i in range(nlayers):
        for ii in range(nnodes[i]):
            tmp+=torch.matmul(torch.matmul(H[i][ii].T,P[i][ii]),H[i][ii])
            #A[i][ii] = 1 / (kalman_lambda + torch.matmul(torch.matmul(H[i][ii].T, P[i][ii]), H[i][ii]))
            #K[i][ii] = torch.matmul(P[i][ii], H[i][ii])
            #K[i][ii] = torch.matmul(K[i][ii], A[i][ii])
    A=1/(kalman_lambda+tmp)
    for i in range(nlayers):
        for ii in range(nnodes[i]):
            K[i][ii] = torch.matmul(P[i][ii], H[i][ii])
            K[i][ii] = torch.matmul(K[i][ii], A)
    get_matrix_time=time.time()
    # print('get_matrix_time: ',get_matrix_time-get_grad_weight_time,'s')
    '''
    2. update weights
    '''
    for i in range(nlayers):
        for ii in range(nnodes[i]):
           #print(weights[i][ii])
            weights[i][ii] += K[i][ii] * error
            #print(weights[i][ii])
    print("NDEKF Etot error: {}".format(error))
    update_weights_time=time.time()
    # print('update_weights_time: ',update_weights_time-get_matrix_time,'s')
    #for i in range(pm.nLayers):
    #    weights[i][0]+=K[i][0]*error
    #    if i==2:
    #        continue
    #    weights[i][1] += K[i][1] * error
    tmp=0
    for i in range(nlayers_embeding):
        for name, param in model.named_parameters():
                if 'embeding_net.weights.weight{}'.format(i) in name:
                    for ii in range(nnodes[tmp]):
                        if ii==0:
                            weights_tmp=weights[tmp][ii][:weights_index[tmp]]
                        else:
                            weights_tmp=torch.cat((weights_tmp,weights[tmp][ii][:weights_index[tmp]]),dim=1)
                    param.data = weights_tmp
    ...
    for i in range(nlayers_resnet_dt):
        for name, param in model.named_parameters():
                if 'fitting_net.resnet_dt.resnet_dt{}'.format(i+1) in name:
                    for ii in range(nnodes[tmp]):
                        if ii==0:
    ...
    # print('save_weights_time: ',save_weights_time-update_weights_time,'s')
    '''
    3. update P
    may be wo should combine all the K,H,P for update K
    '''
    for i in range(nlayers):
        for ii in range(nnodes[i]):
            P[i][ii] = (1 / kalman_lambda) * (P[i][ii] - torch.matmul(torch.matmul(K[i][ii], H[i][ii].T), P[i][ii]))
            P[i][ii] = (P[i][ii] + P[i][ii].T) / 2
    update_P_time=time.time()
    # print('update_P_time: ',update_P_time-save_weights_time,'s')
    '''
    ...
    return P, kalman_lambda

def NDEKF_Force(sample_batches,model,nlayers_embeding,nlayers_fitting,nlayers_resnet_dt,nnodes,kalman_lambda,kalman_nue,P):
    update_weight=1.0
    start=time.time()
    nlayers=nlayers_embeding+nlayers_fitting+nlayers_resnet_dt
    node_num=0
    ...
    # tmp_2=list(range(Force_label.shape[1]))
    tmp_2 = select_list
    '''now it is right'''
    for i in tmp_1:
        for k in tmp_2:
    ...
    for index_i in range(len(random_index)):
        ...
        for index_ii in range(len(random_index[index_i][0])):
            for j in range(3):
                ...
                error_tmp = update_weight*(-1.0*Force_label[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][j] - Force_predict[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][j])
                # error_tmp = update_weight*(Force_predict[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][j] - Force_label[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][j])

                if error_tmp < 0:
                    error_tmp = -1.0 * error_tmp
                    error += error_tmp
                    (update_weight * (-1.0 * Force_predict[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][
                        j])).backward(retain_graph=True)
                else:
                    error += error_tmp
                    (update_weight * (
                    Force_predict[random_index[index_i][0][index_ii]][random_index[index_i][1][index_ii]][j])).backward(
                        retain_graph=True)


        num = len(random_index[index_i][0])
        error = (error / (num * 3.0)) / pm.natoms[0]  
        ...
        for i in range(nlayers_embeding):
            for name, param in model.named_parameters():
                if 'embeding_net.weights.weight{}'.format(i) in name:
                    H[tmp] = list(((param.grad /(num*3.0))/ pm.natoms[0]).chunk(param.grad.shape[1], dim=1))
                    # H[tmp]=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                    weights[tmp]=list(param.data.chunk(param.data.shape[1],dim=1))
                    weights_index.append(param.data.shape[0])
                if 'embeding_net.bias.bias{}'.format(i) in name:
                    H_tmp = list(((param.grad / (num * 3.0)) / pm.natoms[0]).chunk(param.grad.shape[1], dim=1))
                    # H_tmp=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                    weights_tmp = list(param.data.chunk(param.data.shape[1], dim=1))
                    for ii in range(len(H[tmp])):
                        H[tmp][ii]=torch.cat((H[tmp][ii], H_tmp[ii].reshape(-1,1)))
        ...
        for i in range(nlayers_fitting):
            for name, param in model.named_parameters():
                if 'fitting_net.weights.weight{}'.format(i) in name:
                    ...
                    weights_index.append(param.data.shape[0])
                if 'fitting_net.bias.bias{}'.format(i) in name:
                    if param.grad == None:# when name==bias, the grad will be None
                        H_tmp=torch.tensor([[0.0]]).to(device)
                        # tmp_grad=torch.tensor([[0.0]]).to(device)
                        # H_tmp=list((tmp_grad/pm.natoms[0]).chunk(tmp_grad.shape[1],dim=1))
                    else:
                        H_tmp = list(((param.grad / (num * 3.0)) / pm.natoms[0]).chunk(param.grad.shape[1], dim=1))
                        # H_tmp=list((param.grad/pm.natoms[0]).chunk(param.grad.shape[1],dim=1))
                    weights_tmp = list(param.data.chunk(param.data.shape[1], dim=1))
            ...
            for i in range(nlayers):
                for ii in range(nnodes[i]):
                   #print(weights[i][ii])
                    weights[i][ii] += K[i][ii] * error
                    #print(weights[i][ii])
        print("NDEKF Force error: {}".format(error))
        update_weights_time=time.time()
        # print('update_weights_time: ',update_weights_time-get_matrix_time,'s')
        #for i in range(pm.nLayers):
        #    weights[i][0]+=K[i][0]*error
            ...
            for name, param in model.named_parameters():
                    if 'embeding_net.weights.weight{}'.format(i) in name:
                        for ii in range(nnodes[tmp]):
                            if ii==0:
                                weights_tmp = weights[tmp][ii][:weights_index[tmp]]
                                # weights_tmp=weights[tmp][ii][:weights_index[tmp]].T
                            else:
                                weights_tmp = torch.cat((weights_tmp, weights[tmp][ii][:weights_index[tmp]]), dim=1)
                                # weights_tmp=torch.cat((weights_tmp,weights[tmp][ii][:weights_index[tmp]].T))
                        param.data = weights_tmp
                    ...
                    if 'fitting_net.resnet_dt.resnet_dt{}'.format(i+1) in name:
                        for ii in range(nnodes[tmp]):
                            if ii==0:
                                ...
                                weights_tmp=torch.cat((weights_tmp,weights[tmp][ii][:weights_index[tmp]]),dim=1)
                        param.data = weights_tmp#这里似乎有点问题，不用赋值就与weigths_tmp相等了，可能是个隐患
            tmp+=1
        save_weights_time=time.time()
        # print('save_weights_time: ',save_weights_time-update_weights_time,'s')
        '''
...
def train(sample_batches, model, optimizer, criterion, last_epoch, real_lr):
    ...
    # dp 对齐，shift 之后的 label
    # Ei_label = Ei_label - 5133.92272
    # Etot_label = Etot_label - 554464.1386
    # Etot_label = Etot_label - 554463.6064  

    # recover from dpmd
    # dp_force_label = torch.tensor(np.load("/home/husiyu/software/deepMD/deepmd-kit-gpu/dataset/cu/data/deepmd_force_hat.npy"), device=device)
    # dp_etot_label = torch.tensor(np.load("/home/husiyu/software/deepMD/deepmd-kit-gpu/dataset/cu/data/deepmd_etot_hat.npy"), device=device)
    # dp_force_label = torch.tensor(np.load("/home/husiyu/software/MLFFdataset/cu_f1_mini/deepmd_force_hat.npy"), device=device)
...
def valid(sample_batches, model, criterion):
    ...
    loss_F = criterion(Force_predict, -1.0*Force_label)
    loss_Etot = criterion(Etot_predict, Etot_label)
    loss_Ei = criterion(Ei_predict, Ei_label)
...
nlayers_resnet_dt=2
nlayers=nlayers_embeding+nlayers_fitting+nlayers_resnet_dt
nnodes=[0 for i in range(nlayers)]
param_num = [0 for i in range(nlayers)]
for i in range(nlayers_embeding):
    for name, param in model.named_parameters():
            if 'embeding_net.weights.weight{}'.format(i) in name:
                param_num[tmp_n] = param.data.shape[0]
                nnodes[tmp_n]=param.data.shape[-1]
                tmp_n+=1
                break
for i in range(nlayers_fitting):
    for name, param in model.named_parameters():
...
P = [[[] for ii in range(nnodes[i])] for i in range(nlayers)]
for i in range(nlayers):
    for ii in range(nnodes[i]):
        if i<nlayers_embeding+nlayers_fitting:
            P[i][ii] = (torch.eye(param_num[i] + 1)).to(device)
        else:
            P[i][ii] = (torch.eye(param_num[i])).to(device)
#======================================
...
for epoch in range(start_epoch, n_epoch + 1):
    ...
    for i_batch, sample_batches in enumerate(loader_train):
        ...
        P,kalman_lambda=NDEKF_Etot(sample_batches,model,nlayers_embeding,nlayers_fitting,nlayers_resnet_dt,nnodes,kalman_lambda,kalman_nue,P)
        P, kalman_lambda = NDEKF_Force(sample_batches, model, nlayers_embeding, nlayers_fitting, nlayers_resnet_dt,nnodes, kalman_lambda, kalman_nue, P)

        batch_loss, batch_loss_Etot, batch_loss_Ei, batch_loss_F = valid(sample_batches, model, nn.MSELoss())

        end = time.time()
        print("total NDEKF time:", end-start, 's')
