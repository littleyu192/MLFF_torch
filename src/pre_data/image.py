...
class Image():
    ...
    def __init__(self,
        ...
        if atomCountAsType is None:
            self.atomCountAsType=np.array([self.atomTypeList.count(i) for i in self.atomTypeSet])
        else:
            self.atomCountAsType=atomCountAsType
        ...
        if atomCategoryDict is None:
            ...
            for atomType in pm.atomType:
                self.atomCategoryDict[atomType]=np.where(np.array(self.atomTypeList)==atomType)[0]
    ...
    def checkCell(self):
        ...
        if True:
            latticeConstants=np.linalg.norm(self.cupyCell,axis=1)
            volume=np.abs(np.linalg.det(self.cupyCell))
            idx0=np.arange(3)
            idxm=(idx0-1)%3
            idxp=(idx0+1)%3
            cosAng[idx0]=np.sum(self.cupyCell[idxm]*self.cupyCell[idxp],axis=1)/(latticeConstants[idxm]*latticeConstants[idxp])
            '''
            ...
            '''
            sinAng=np.sqrt(1.0-cosAng*cosAng)
    ...
    def calDistanceVectArray(self,isShiftZeroPoint=False,shiftZeroPointVect=np.array([0.0,0.0,0.0])):
        '''
    ...
    def calCellNeighborStruct(self,shiftVect=np.zeros((3,)),shiftOrder=0,rMin=pm.rMin,rCut=pm.Rc_M):
        '''
    ...
    def calAllNeighborStruct(self,isSave=False,isCheckFile=True,rMin=pm.rMin,rCut=pm.Rc_M):
        ...
        self.neighborIndexOfCenterAtoms=np.array([],cp.int32)        #此二处必须加上dtype,否则默认dtype是cp.float64
        self.neighborIndexOfNeighborAtoms=np.array([],cp.int32)      #
