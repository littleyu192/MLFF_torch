...
cccc around each atom, there is a group
cccc We only fit group energy, instead of each individual atom energy
cccccc here, the feat in group is the combined feature (j,itype)
       feat2_group=0.d0
       ...
       sum=sum+fact
       do j=1,nfeat2(itype)
       feat2_group(j+iii,iat1)=feat2_group(j+iii,iat1)+ 
     &          feat2_type(j,num(itype),itype)*fact
       enddo
       enddo
...
!200    continue
     ...
     & 1.d0,feat2_group,nfeat2tot,feat2_group,nfeat2tot,
     & 0.d0,AA_tmp,nfeat2tot)
