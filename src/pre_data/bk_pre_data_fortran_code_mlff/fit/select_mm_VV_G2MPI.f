...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       S=0.d0
       call dgemm('N','T',num_ref,num_ref,ncase,1.d0,
     & feat_case,nfeat,
     & feat_case,nfeat,0.d0,S,num_ref)         ! feat_case is (nfeat0,ncase)

       allocate(S_tmp(num_ref,num_ref))
       call mpi_allreduce(S,S_tmp,num_ref**2,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       S=S_tmp
       deallocate(S_tmp)
       ...
       call mpi_allreduce(BB,BB_tmp,num_ref,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       BB=BB_tmp
       deallocate(BB_tmp)
!cccccccccccc
...
!cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       enddo
       call mpi_allreduce(diff1,diff1tmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       diff1=diff1tmp
       call mpi_allreduce(diff2,diff2tmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       diff2=diff2tmp
       call mpi_allreduce(diff4,diff4tmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       diff4=diff4tmp
...
!       write(6,*) "iloop",iloop1,iloop2
       if(inode.eq.1) then
       write(6,"('diff1,2,4(Lin)     =',3(E14.7,1x))") 
     &  diff1a,diff2a,diff4a
       endif
...
!       write(6,*) "iloop",iloop1,iloop2
       if(inode.eq.1) then
       write(6,"('diff1,2,4(Lin,test)=',3(E14.7,1x))") 
     &    diff1t,diff2t,diff4t
       endif
...
ccccccc  ind_kkk_kkk0(kkk)=kkk0
       ...
       if(inode.eq.1) then
       open(33,file="VV_select_conv.plot")
       rewind(33)
       loop=0
...
ccccccccccccccccccccccccccccccccc
cccc  At this state, iflag_selected(kkk0).eq.1, selected
       kkk0=0
       kkk=0
       ...
       call mpi_allreduce(sum1,sum1tmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       sum1=sum1tmp
       call mpi_allreduce(sum2,sum2tmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       sum2=sum2tmp
...
!   also the dE_sum
       ...
       j3=idd(3,kkk0)
       do i=1,ncase
       feat_new(iii,i)=feat_ext3(i,j1,1)*feat_ext3(i,j2,1)*
     &   feat_ext3(i,j3,1)
       enddo
       endif
...
! cannot normalize the feature, that is wrong !, keep the feature untouched
       ...
       call mpi_allreduce(sum,sumtmp,1,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       sum=sumtmp
       ...
       S=0.d0
       call dgemm('N','T',mm4,mm4,ncase,1.d0,
     & feat_new,mm4,feat_new,mm4,0.d0,S,mm4)

       allocate(S_tmp(mm4,mm4))
       call mpi_allreduce(S,S_tmp,mm4**2,MPI_REAL8,MPI_SUM,
     &    MPI_COMM_WORLD,ierr)
       S=S_tmp
       deallocate(S_tmp)
...
!       write(6,*) "num_ref=",num_ref
       ...
       S=0.d0
       call dgemm('N','T',num_ref,num_ref,ncase,1.d0,
     & feat_new,num_ref,feat_new,num_ref,0.d0,S,num_ref)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &    MPI_COMM_WORLD,ierr)
       ...
       if(inode.eq.1) then
       write(6,"('TEST diff1,2,4=     ',3(E14.7,1x))") 
     &   diff1t,diff2t,diff4t
