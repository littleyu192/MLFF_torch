...
!       real*8 xp(5,100),xp1(10,100)
     ...
     &   MOVEMENTallDir
       ...
       integer, allocatable, dimension (:,:) :: iflag_selected,
     &     ind_kkk_kkk0
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       open(12,file="OUT.xp",form="unformatted")
       rewind(12)
       read(12) mfeat2,ntype_t,ndim_t,ndim1_t
       if(ntype_t.ne.ntype.or.ndim_t.ne.ndim.or.ndim1_t.ne.ndim1) then
       write(6,*) "Inconsistent ntype,ndim1,ndim from OUT.xp",
     &       ntype_t,ndim1_t,ndim_t
       stop
       endif
       read(12) nfeat2
...
ccccccc  ind_kkk_kkk0(kkk)=kkk0
       ...
       enddo !itype
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccc In the following, we will select mm3 feature from mm(itype)


ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
CCC note, the mm(itype) will change from iloop2=1, to iloop2=2
cc for iloop2=2, mm already changed to mm4, and idd has been changed as the selected mm4 features
...
cccccccccccccccccccccccccccccccccccccccccc
       ...
       open(90,file="energyVV.pred.group")  ! fitting result from last iteration
       rewind(90)
...
!  This inp(kkk) alway at the position before the next energy
       ...
       write(6,"('iloop,iloop2,sys,image=',4(i6,1x))")
     &  iloop,iloop2,sys,image
       endif
...
!-----------------------------------------------------------------
       ...
       inp(kkk)=inp(kkk)+(4*natom+nfeat1tm(kkk)*natom)*8+
     &    (natom+m_neigh*natom)*4    ! jump from eenrgy to num_tmp
...
ccccccccccccccccccccccccccccccccccccccccccc
     ...
     & nfeat1(itype),1.d0,PV(1,1,itype),
...
!       call dgemm('T','N',nfeat2(itype),num_atomtype(itype),
       call dgemm('T','N',nfeat2(itype),num(itype),
     &  nfeat1(itype),1.d0,PV(1,1,itype),
...
!       feat_new(kkk+nfeat2(itype),i)=feat_ext2(i,j1,id1)*
       feat_new(kkk,i)=feat_ext2(i,j1,id1)*
     &              feat_ext2(i,j2,id2)
...
!       feat_new(kkk+nfeat2(itype),i)=feat_ext3(i,j1,1)*
       feat_new(kkk,i)=feat_ext3(i,j1,1)*
...
! speed of this step. 
        sum=sum+fact
        do j=1,mm(itype)
        feat_new_group(j+iii,iat1)=feat_new_group(j+iii,iat1)+
     &          feat_new(j,iat2)*fact
        enddo
        endif
...
!       enddo
       ...
       do itype=1,ntype
       call dgemm('N','T',mm_max,mm_max,num_atomtype(itype),1.d0,
     & feat_new_type(1,1,itype),mm_max,feat_new_type(1,1,itype),
     &  mm_max,1.d0,
     & S1(1,1,itype),mm_max)
       enddo
...
!       dfeat_new(kkk+nfeat2(itype),i,jj,:)=dfeat_ext1(ii,j1,id1)*
       dfeat_new(kkk,i,jj,:)=dfeat_ext1(ii,j1,id1)*
     &      dfeat2(j1,i,jj,:)
...
!       dfeat_new(kkk+nfeat2(itype),i,jj,:)=dfeat_ext2(ii,j1,id1)*
       dfeat_new(kkk,i,jj,:)=dfeat_ext2(ii,j1,id1)*
...
!       dfeat_new(kkk+nfeat2(itype),i,jj,:)=dfeat2(j1,i,jj,:)*
       dfeat_new(kkk,i,jj,:)=dfeat2(j1,i,jj,:)*
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       do j=1,num_ref(itype)
       do i=1,natom
       dF_term(j,ixyz,itype)=dF_term(j,ixyz,itype)+dF_prev(i,ixyz)*
     &                                             SS(j,i,ixyz,itype)
...
! S2 is not correct,  need to use NfeatNtot !
       ...
       do itype2=1,ntype
       do ixyz=1,3
       call dgemm('N','T',mm_max,mm_max,natom,1.d0,
     & SS(1,1,ixyz,itype1),mm_max,SS(1,1,ixyz,itype2),mm_max,1.d0,
     & S2(1,1,ixyz,itype1,itype2),mm_max)
       enddo
       enddo
       enddo
       endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccc Now, select mm4 from the mm(itype)
       call mpi_barrier(MPI_COMM_WORLD,ierr)
       allocate(dE_tmp(num_refm,ntype))
       allocate(dF_tmp(num_refm,3,ntype))
        call mpi_allreduce(dE_term,dE_tmp,num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        dE_term=dE_tmp
        call mpi_allreduce(fE_term,dE_tmp,num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        fE_term=dE_tmp
        call mpi_allreduce(dE_tot,dE_tmp,num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        dE_tot=dE_tmp
        call mpi_allreduce(fE_tot,dE_tmp,num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        fE_tot=dE_tmp
        call mpi_allreduce(dF_term,dF_tmp,3*num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        dF_term=dF_tmp
        call mpi_allreduce(fF_term,dF_tmp,3*num_refm*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
        fF_term=dF_tmp
       deallocate(dE_tmp)
       deallocate(dF_tmp)
...
!     &     +dF_term(kkk,3,itype)**2)/fF_term(kkk,itype)*weight_F
       dtot_term(kkk,itype)=dE_term(kkk,itype)*weight_E+
     &      (dF_term(kkk,1,itype)+dF_term(kkk,2,itype)
     &     +dF_term(kkk,3,itype))*weight_F
     &     +dE_tot(kkk,itype)*weight_E0
       dtot_sum(kkk,itype)=fE_term(kkk,itype)*weight_E+
     &     +(fF_term(kkk,1,itype)+fF_term(kkk,2,itype)+
     &       +fF_term(kkk,3,itype))*weight_F+
     &       +fE_tot(kkk,itype)*weight_E0
       dtot_sum(kkk,itype)=abs(dtot_term(kkk,itype)**2/
     &                         dtot_sum(kkk,itype))
       enddo
       enddo
...
!  use S matrix
       ...
       call mpi_barrier(MPI_COMM_WORLD,ierr)
        ...
        call mpi_allreduce(S1,S1_tmp,mm_max**2*ntype,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
         S1=S1_tmp
        call mpi_allreduce(S2,S2_tmp,3*mm_max**2*ntype**2,MPI_REAL8,
     &   MPI_SUM,MPI_COMM_WORLD,ierr)
         S2=S2_tmp
         deallocate(S1_tmp)
         deallocate(S2_tmp)
       ...
       do itype1=1,ntype
       do itype2=1,ntype
       S(:,:,itype1,itype2)=S(:,:,itype1,itype2)+
     &  (S2(:,:,1,itype1,itype2)+S2(:,:,2,itype1,itype2)+
     &   S2(:,:,3,itype1,itype2))*weight_F
       enddo
       enddo
...
!     &         S2(:,:,2,itype)+S2(:,:,3,itype))*weight_F
         ...
         iii_max=1
         do iii=1,mm4
         if(dtot_term(iii,itype)**2/S(iii,iii,itype,itype)
     &       .gt.dE_max) then
         ...
         do itype2=1,ntype
         do iii=1,mm4
         dtot_term(iii,itype2)=dtot_term(iii,itype2)-
     &    S(iii,iii_max,itype2,itype)*bc    ! the change of dE_sum due to the substract of iii_max
...
!cccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       write(12,"(5(i5,1x),2x,i8)") 
     &     (idd3(jj,ii,itype),jj=0,4),kkk0_st2(ii,itype)
       enddo
       close(12)
       enddo
       endif
