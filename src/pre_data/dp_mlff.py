...
def sepper_data(config, davg_path=None):
    trainset_dir = config["trainSetDir"]
    ...
    if davg_path is None:
        ...
        davg = np.load(os.path.join(davg_path, "davg.npy"))
        dstd = np.load(os.path.join(davg_path, "dstd.npy"))
...
def al_main():
    ...
    parser.add_argument('-c', '--config', help='specify config path such as template.yaml', type=str, default='cu_config_template.yaml')
    parser.add_argument('-w', '--workdir', help='specify the work dir: ./PWdata/MOVEMENT', type=str, default='.')
    parser.add_argument('-d', '--davg-path', help='specify the dir of davg.npy or dstd.npy, if None, they will generated by current MOVEMENT', type=str, default=None)
    args = parser.parse_args()

    config_path = args.config
    cwd = args.workdir
    davg_path = args.davg_path
