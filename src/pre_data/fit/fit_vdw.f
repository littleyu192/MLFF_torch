...
!     &   -(pi/(2*rad))*cos(yy)*sin(yy))
       ...
       dEdd=0.5*4*(-12*(rad/dd)**12/dd*cos(yy)**2
     &   -(pi/(2*rad))*cos(yy)*sin(yy)*(rad/dd)**12)
       dEdd2=0.5*4*(-6*(rad/dd)**6/dd*cos(yy)**2
...
! itype1 is the center atom
       AA_vdw(it1,itype1,j1,it2,itype1,j2)=
     &    AA_vdw(it1,itype1,j1,it2,itype1,j2)+
     &      (sum_vdw(it1,j1)*sum_vdw(it2,j2)
     &               +w_f*sumF_vdw(1,it1,j1)*sumF_vdw(1,it2,j2)
     &               +w_f*sumF_vdw(2,it1,j1)*sumF_vdw(2,it2,j2)
     &               +w_f*sumF_vdw(3,it1,j1)*sumF_vdw(3,it2,j2))*w_vdw      ! 0.2 is the weight for force
       AA_vdw(itype1,it1,j1,itype1,it2,j2)=
     &    AA_vdw(itype1,it1,j1,itype1,it2,j2)
     &               +(w_f*sumF_vdw(1,it1,j1)*sumF_vdw(1,it2,j2)
     &                +w_f*sumF_vdw(2,it1,j1)*sumF_vdw(2,it2,j2)
     &                +w_f*sumF_vdw(3,it1,j1)*sumF_vdw(3,it2,j2))*w_vdw      ! 0.2 is the weight for force
       AA_vdw(it1,itype1,j1,itype1,it2,j2)=
     &   AA_vdw(it1,itype1,j1,itype1,it2,j2)
     &               +(w_f*sumF_vdw(1,it1,j1)*sumF_vdw(1,it2,j2)
     &                +w_f*sumF_vdw(2,it1,j1)*sumF_vdw(2,it2,j2)
     &                +w_f*sumF_vdw(3,it1,j1)*sumF_vdw(3,it2,j2))*w_vdw      ! 0.2 is the weight for force
       AA_vdw(itype1,it1,j1,it2,itype1,j2)=
     &    AA_vdw(itype1,it1,j1,it2,itype1,j2)
     &               +(w_f*sumF_vdw(1,it1,j1)*sumF_vdw(1,it2,j2)
     &                +w_f*sumF_vdw(2,it1,j1)*sumF_vdw(2,it2,j2)
     &                +w_f*sumF_vdw(3,it1,j1)*sumF_vdw(3,it2,j2))*w_vdw      ! 0.2 is the weight for force
       ...
       do j1=1,nterm
       do it1=1,ntype
       BB_vdw(it1,itype1,j1)=BB_vdw(it1,itype1,j1)+
     &        (energy(i)*sum_vdw(it1,j1)
     ...
     &       +w_f*force(2,i)*sumF_vdw(2,it1,j1)
     &       +w_f*force(3,i)*sumF_vdw(3,it1,j1))*w_vdw
       BB0_vdw(it1,itype1,j1)=BB0_vdw(it1,itype1,j1)+
     &        sum_vdw(it1,j1)*w_vdw     ! this is only for energy
       enddo
       enddo
...
900    continue
       ...
       do itype1=1,ntype
       do it1=1,ntype
       BB_vdw(it1,itype1,j1)=BB_vdw(it1,itype1,j1)-E_ave_vdw(itype1)*
     &   BB0_vdw(it1,itype1,j1)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       do itype1=1,ntype
       do it1=1,ntype
       AA_vdw(it1,itype1,j1,it1,itype1,j1)=
     &   AA_vdw(it1,itype1,j1,it1,itype1,j1)+1.D-8
       enddo
       enddo
       enddo

       call dgesv(ntype**2*nterm,1,AA_vdw,ntype**2*nterm,ipiv,
     &  BB_vdw,ntype**2*nterm,info)
...
!       enddo
       ...
       if(iloop.eq.1) then
       open(10,file="vdw_fitB.ntype0")
       rewind(10)
       write(10,*) ntype,nterm
       do itype1=1,ntype
       write(10,"(i4,2x,2(f10.5,2x),2x,20(E13.6,1x))") 
     &   itype_atom(itype1),
     &   rad_atom(itype1),E_ave_vdw(itype1),
     &    ((BB_vdw(it1,itype1,j1),it1=1,ntype),j1=1,nterm)
       ...
       write(10,*) ntype,nterm
       do itype1=1,ntype
       write(10,"(i4,2x,2(f10.5,2x),2x,10(E13.6,1x))") 
     &   itype_atom(itype1),
     &   rad_atom(itype1),E_ave_vdw(itype1),
     &    ((BB_vdw(it1,itype1,j1),it1=1,ntype),j1=1,nterm)
       enddo
