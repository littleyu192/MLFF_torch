...
def train_kalman(sample_batches, model, optimizer, criterion, last_epoch, real_lr):
    ...
    KFOptWrapper = KFOptimizerWrapper(
        model, optimizer, 24, 6, None, "torch"
    )
...
if pm.use_GKalman == True:
    optimizer = GKFOptimizer(
            model.parameters(), 0.98, 0.9987, device, opt_dtype
    )
...
for epoch in range(start_epoch, n_epoch + 1):
    ...
    for i_batch, sample_batches in enumerate(loader_train):
        ...
        for param_group in optimizer.param_groups:
            ...
            param_group['lr'] = real_lr * (opt_batch_size  ** 0.5)
        
        natoms_sum = sample_batches['natoms_img'][0, 0].item()
        
        if (pm.use_GKalman == True) or (pm.use_LKalman == True):
            real_lr = 0.001
    ...
    if RMSE_Etot + RMSE_F < min_loss:
            train_loss = RMSE_Etot + RMSE_F
            min_loss = train_loss
            works_epoch = 0
