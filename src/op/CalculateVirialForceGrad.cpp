    ...
    int64_t neigh_num, const torch::Tensor &grad)
{
    ...
    if (dtype == torch::kFloat32)
    {
        launch_calculate_virial_force_grad(
            nblist.data_ptr<int>(), Rij.data_ptr<float>(), Ri_d.data_ptr<float>(),
            net_grad.data_ptr<float>(), batch_size, natoms, neigh_num,
            grad.data_ptr<float>());
    }
    else if (dtype == torch::kFloat64)
    {
        launch_calculate_virial_force_grad(
            nblist.data_ptr<int>(), Rij.data_ptr<double>(), Ri_d.data_ptr<double>(),
