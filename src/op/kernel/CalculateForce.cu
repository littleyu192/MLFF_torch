...
__global__ void force_deriv_wrt_neighbors_a(
    double *force, const double *net_deriv, const double *in_deriv, const int *nlist,
...
__global__ void force_calc(
    DType *force, const DType *net_deriv, const DType *in_deriv, const int *nlist,
    const int nloc, const int nnei)
{
    ...
    const unsigned int nlist_offset =
        batch_id * nloc * nnei + atom_id * nnei + neigh_index;
    ...
    const unsigned int net_offset =
        batch_id * nloc * ndescrpt + atom_id * ndescrpt + neigh_index * 4;
    const unsigned int in_offset =
        batch_id * nloc * ndescrpt * 3 + atom_id * ndescrpt * 3 + neigh_index * 12;
...
template void launch_calculate_force(
    const int *nblist, const float *dE, const float *Ri_d, const int batch_size,
    const int natoms, const int neigh_num, float *force);

template void launch_calculate_force(
    const int *nblist, const double *dE, const double *Ri_d, const int batch_size,
    const int natoms, const int neigh_num, double *force);
