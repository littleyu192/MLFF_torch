...
__device__ inline DType dev_dot(const DType *arr1, const DType *arr2)
{
...
__global__ void force_grad_wrt_center_atom(
    DType *grad_net, const DType *grad, const DType *env_deriv, const int natoms,
    const int ndescrpt)
{
    ...
    const int env_deriv_offset =
        batch_id * natoms * ndescrpt * 3 + center_idx * ndescrpt * 3 + descrpt_idx * 3;
...
__global__ void force_grad_wrt_neighbors_a(
    DType *grad_net, const DType *grad, const DType *env_deriv, const int *nlist,
    const int nloc, const int nnei)
{
    // idy -> nnei
    ...
    if (j_idx >= nloc)
        j_idx = j_idx % nloc;
    const unsigned int grad_net_offset =
        batch_id * nloc * nnei * 4 + idx * nnei * 4 + idy * 4 + idw;
    const unsigned int grad_offset = batch_id * nloc * 3 + j_idx * 3;
    const unsigned int env_deriv_offset =
        batch_id * nloc * nnei * 4 * 3 + idx * nnei * 4 * 3 + idy * 4 * 3 + idw * 3;
    grad_net[grad_net_offset] +=
        dev_dot(grad + grad_offset, env_deriv + env_deriv_offset);
}
    ...
    const int natoms, const int neigh_num, DType *grad)
{
    ...
    force_grad_wrt_center_atom<<<block_grid, thread_grid>>>(
        grad, net_grad, Ri_d, natoms, ndesc);
    ...
    force_grad_wrt_neighbors_a<<<block_grid_, thread_grid_>>>(
        grad, net_grad, Ri_d, nblist, natoms, neigh_num);
}

template void launch_calculate_force_grad(
    const int *nblist, const float *Ri_d, const float *net_grad, const int batch_size,
    const int natoms, const int neigh_num, float *grad);

template void launch_calculate_force_grad(
    const int *nblist, const double *Ri_d, const double *net_grad, const int batch_size,
    const int natoms, const int neigh_num, double *grad);
