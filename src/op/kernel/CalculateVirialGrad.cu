...
__global__ void virial_grad_wrt_neighbors_a(
    ...
    const DType *Rij, const int *nlist, const int natoms, const int neigh_num)
{
    ...
    int j_idx =
        nlist[batch_id * natoms * neigh_num + atom_id * neigh_num + neigh_index];
    ...
    DType tmp[9];
    const int Rij_offset =
        batch_id * natoms * neigh_num * 3 + atom_id * neigh_num * 3 + neigh_index * 3;
    const int Ri_d_offset = batch_id * natoms * neigh_num * 4 * 3 +
                            atom_id * neigh_num * 4 * 3 + neigh_index * 4 * 3 +
                            index_xyzw * 3;
    ...
    const int grad_output_offset = batch_id * natoms * neigh_num * 4 +
                                   atom_id * neigh_num * 4 + neigh_index * 4 +
                                   index_xyzw;
...
void launch_calculate_virial_force_grad(
    ...
    const int batch_size, const int natoms, const int neigh_num, DType *grad_output)
{
    int LEN = 128;
    ...
    virial_grad_wrt_neighbors_a<<<block_grid, thread_grid>>>(
        grad_output, net_grad, Ri_d, Rij, nblist, natoms, neigh_num);
}

template void launch_calculate_virial_force_grad(
    const int *nblist, const float *Rij, const float *Ri_d, const float *net_grad,
    const int batch_size, const int natoms, const int neigh_num, float *grad);

template void launch_calculate_virial_force_grad(
    const int *nblist, const double *Rij, const double *Ri_d, const double *net_grad,
    const int batch_size, const int natoms, const int neigh_num, double *grad);
