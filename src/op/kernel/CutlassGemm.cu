    ...
    double const *const *bias, double beta, int batch_count)
{
    ...
    using Swizzle =
        typename cutlass::gemm::threadblock::GemmBatchedIdentityThreadblockSwizzle;
    ...
    using EpilogueOutputOp =
        typename cutlass::epilogue::thread::LinearCombinationGeneric<
            cutlass::epilogue::thread::Tanh, double, 1, double, double>;
    ...
    //     cutlass::arch::OpClassTensorOp, cutlass::arch::Sm80, ThreadblockShape, WarpShape, InstructionShape,
    //     EpilogueOutputOp, Swizzle, kStages, kAlignmentA, kAlignmentB, Operator>;

    using Gemm = cutlass::gemm::device::GemmArray<
        double, RowMajor, double, RowMajor, double, RowMajor, double,
        cutlass::arch::OpClassSimt, cutlass::arch::Sm80, ThreadblockShape, WarpShape,
        InstructionShape, EpilogueOutputOp, Swizzle, kStages, kAlignmentA, kAlignmentB,
        Operator>;

    using Argument = Gemm::Arguments;

    Argument args(
        {m, n, k}, A, lda, B, ldb, bias, 0, C, ldc, {alpha, beta}, batch_count);
...
{
    using Epilogue = typename cutlass::epilogue::thread::LinearCombinationGeneric<
        cutlass::epilogue::thread::Tanh, double, 1, double, double>;
};
...
{
    using Epilogue = typename cutlass::epilogue::thread::LinearCombination<
    ...
    double *D, int ldd, int strideD, double beta, int batch_count, bool tanh)
{
    ...
    if (tanh)
    {
        ...
        using Gemm = cutlass::gemm::device::GemmBatched<
            double, RowMajor, double, RowMajor, double, RowMajor, double,
            cutlass::arch::OpClassSimt, cutlass::arch::Sm80, ThreadblockShape,
            WarpShape, InstructionShape, EpilogueOutputOp, Swizzle, kStages,
        ...
        Argument args(
            ...
            {D, ldd}, strideD, {alpha, beta}, batch_count);
